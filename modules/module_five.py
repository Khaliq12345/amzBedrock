    ...
    for i, row in input_df.iterrows():
        ...
        # option 3
            ...
            for cat, cat_name in zip(targets[0], targets[1]):
                ...
                if 'views-Category' in row['Audience Targeting']:
                    targeting_exp_value = f"views-category-[{cat}]-lookback-{row['Lookback']}"
                elif 'purchases-Category' in row['Audience Targeting']:
                    targeting_exp_value = f"purchases-category-[{cat}]-lookback-{row['Lookback']}"
                else:
                    targeting_exp_value = None
                x_table.loc[x_table['Entity'] == 'Audience Targeting', 'Targeting Expression'] = targeting_exp_value
                #campaign name
                cost_type = cost_type_selector(row['Bid Optimization'])
                campaign_name = f"SD_{row['SKU']}_{row['ASIN']}_{cost_type}_{targeting_exp_value}_Retargeting"
        ...
        #option 4
            ...
            if 'views-Advertised products' in row['Audience Targeting']:
                targeting_exp_value = f"views-exact-product lookback-{row['Lookback']}"
            elif 'views-Related to advertised products' in row['Audience Targeting']:
                targeting_exp_value = f"views-similar-product lookback-{row['Lookback']}"
            elif 'purchases-Advertised products' in row['Audience Targeting']:
                targeting_exp_value = f"purchases-exact-product lookback-{row['Lookback']}"
            elif 'views-Related to advertised products' in row['Audience Targeting']:
                targeting_exp_value = f"purchases-related-product lookback-{row['Lookback']}"
            x_table.loc[x_table['Entity'] == 'Audience Targeting', 'Targeting Expression'] = targeting_exp_value
            #campaign name
            cost_type = cost_type_selector(row['Bid Optimization'])
            campaign_name = f"SD_{row['SKU']}_{row['ASIN']}_{cost_type}_{targeting_exp_value}_Retargeting"
